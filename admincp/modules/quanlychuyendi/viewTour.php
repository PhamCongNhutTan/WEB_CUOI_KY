...
<a style="text-decoration: none;" href="?action=quanlychuyendi&query=add">
    <button id="create" type="button" class="btn btn-success text-success ms-2 mt-3" style="background-color: inherit">
        <i class="bi bi-plus-circle text-success"></i><a style="text-decoration: none; color: green;" href="?action=quanlychuyendi&query=add"> Thêm chuyến đi</a>
    </button>
</a>
<div class="mt-3">
    ...
    <select class="form-select" style="width: 25%; padding: 4px;" name="type" id="" onchange="handleSelect(this);">
        <option value="Không" <?php if($type == "khong" || $type == "") echo "selected"?> >Không</option>
        <option value="Đang mở" <?php if($type == "dangmo") echo "selected"?>>Đang mở</option>
        <option value="Đang đóng" <?php if($type == "dangdong") echo "selected"?>>Đang đóng</option>
    </select>
</div>
    ...
    <tbody>
        ...
        ?>
            <tr class="py-3">
                ...
                <td>
                    <img style="width: 100%; max-width: 300px;" class="border-2 rounded" src="<?php echo "../images/tour/" . $tour->getTourID() . "/" . $tour->getImagePath() ?>" width="100px" alt="">
                </td>
                <td style="text-wrap: nowrap;"><?php echo ($tour->getType() ?? "Không có") ?></td>
                <td>
                    <button type="button" class="btn btn-primary"> <a style="text-decoration:none; color:white;" href="?action=quanlychuyendi&query=edit&tour_id=<?php echo $tour->getTourID() ?>"><i class="bi bi-pencil"></i></a></button>
                </td>
            </tr>
        <?php
        }
